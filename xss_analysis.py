...
# Function to check if node is unsanitized output
def is_unsanitized_output(node):
    # Implement logic to check if node is directly output without proper sanitization
    # This might involve analyzing parent nodes and their context
    return True
def search_for_xss(node):
    # print(node)
    response = []
    if isinstance(node, dict):
        if node.get('nodeType') is None:
            # print('nodeType is not found')
            x = 1
        else:
            node_type = node['nodeType']

            # Check for user input accessing global arrays
            if node_type == 'global' and node['name'] in ['$_GET', '$_POST', '$_REQUEST']:
                # Check if this user input is directly echoed or printed
                if is_unsanitized_output(node):
                    # print("Potential XSS vulnerability detected:", node)
                    response.append(node)
            
            # Recurse through child nodes
            for key, value in node.items():
                search_for_xss(value)
...
def split_multiple_statements(line):
    # Split the line by semicolons to separate statements
    statements = line.split(';')

    # Filter out empty statements and whitespace
    resp_array = [statements[0] + ';']
    for statement in statements[1:-1]:
        resp_array.append('  ' + statement.strip() + ';')
    resp_array.append(statements[-1].strip())
    # statements = [ statement.strip() + ';' for statement in statements if statement.strip()]
    print(resp_array)
    response = "\n".join(resp_array)
    # Return statements as separate lines
    return response

class PHPSimpleVisitor(ast.NodeVisitor):
    def visit(self, node):
        print(node.__class__.__name__)
        self.generic_visit(node)
# Define a custom visitor class to traverse AST
class XssVisitor(ast.NodeVisitor):
    ...
    def visit_BinOp(self, node):
        if isinstance(node.op, ast.Add):
            left = self._get_node_as_string(node.left)
            right = self._get_node_as_string(node.right)

            if self._is_user_input(left) or self._is_user_input(right):
                self.vulnerabilities.append(node)

        self.generic_visit(node)

    def _get_node_as_string(self, node):
        return ast.dump(node)

    def _is_user_input(self, text):
        return re.search(r'\$_(GET|POST|REQUEST)', text) is not None
...
# Recreate AST nodes
def reconstruct_ast_node(node_json):
    node_type = node_json["node_type"]

    if node_type == "Program":
        return php_ast.Program(body=[reconstruct_ast_node(child) for child in node_json["body"]])
    elif node_type == "Assignment":
        return php_ast.Assignment(left=reconstruct_ast_node(node_json["left"]),
                                  right=reconstruct_ast_node(node_json["right"]))
    elif node_type == "Variable":
        return php_ast.Variable(name=node_json["name"])
    elif node_type == "Number":
        return python_ast.Constant(value=node_json["value"])
...
# Web Malware Investigation
        if not repo.bare:
                ...
                for file in files:
                    if ('.php' in file.filename) and ('.php.' not in file.filename):
                        ...
                        try:    # Generate AST for Analysis Passes
                            ...
                            try:
                                php_ast = subprocess.check_output(cmd)
                            
                            
                            except:
                                print('could not parse php: ', file.filepath)
                                continue
                                #sys.exit(1)
                            ...
                            try:
                                # php_ast = php_ast.decode().replace('"', '\\"')
                                # php_ast = php_ast.decode().replace("'", '"')
                                # php_ast = php_ast.replace('false', '\"FALSE\"')
                                # php_ast = php_ast.replace('true', '\"TRUE\"')
                                php_ast = json.loads(php_ast)

                                # print(php_ast)
                                # print('************************************')
                                #print('begin literal')
                                #php_ast = ast.literal_eval(parsed_ast)
                                with open('results/ast.json', 'w') as json_file:
                                    json.dump(php_ast, json_file, indent=4)
                                
                                vulnerable_nodes = []
                                for node in php_ast:
                                    vulnerable_nodes + search_for_xss(node)

                                if len(vulnerable_nodes) > 0:
                                    result[c_obj.commit_id].append(file.filepath)

                                #print(json.dumps(reconstructed_ast, indent=4))
                            except  Exception as e:
                                print("ENCOUNTERED EXCEPTION WHILE LOADING {}".format(file.fileath))
                                continue
                                # print(php_ast)
                                # sys.exit(1)
                            ...
                            print("ENCOUNTERED EXCEPTION FOR {}".format(file.filepath))
                            continue
                            # sys.exit(1)
            ...
            with open('results/ast_result.json', 'w') as json_file:
